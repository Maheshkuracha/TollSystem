...
def CollectPaymentAction(request):
    ...
    if request.method == 'POST':
        ...
        if balance > 0:
            ...
            student_sql_query = "update recharge set available_amount = available_amount - 20, transaction_status='Toll Amount Deducted', transaction_date="+str(today)+" where vehicle_no='"+res2+"'"
            ...
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'tollgate',charset='utf8')
            db_cursor = db_connection.cursor()
            student_sql_query = "INSERT INTO payments(vehicle_no,amount,payment_date) VALUES('"+res2+"','20','"+str(today)+"')"
            db_cursor.execute(student_sql_query)
            db_connection.commit()
        context= {'data':output}
...
def ViewBalance(request):
    if request.method == 'GET':
        ...
        font = '<font size="" color="black">'
        output += "<tr>"
        dataset_columns = ['Username', 'Vehicle', 'Available Amount', 'Last Transaction Status', 'Last Transaction Date']
        output += "<tr>"
        for i in range(len(dataset_columns)):
            output += "<th>"+font+dataset_columns[i]+"</th>"
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'tollgate',charset='utf8')
